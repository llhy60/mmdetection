...
class QuadXMLDataset(CustomDataset):
    ...
    def get_ann_info(self, idx):
        ...
        for obj in root.findall('object'):
            ...
            quadbox = obj.find('polygon')
            bnd_box = obj.find('bndbox')
            if quadbox:
                quad_bbox = [eval(points) for points in quadbox.find('points').text.split(";")]
                if len(quad_bbox) == 6:
                    quadx0, quady0 = quad_bbox[0]
                    quadx1, quady1 = quad_bbox[1]
                    quadx2, quady2 = quad_bbox[3]
                    quadx3, quady3 = quad_bbox[4]
                else:
                    quadx0, quady0 = quad_bbox[0]
                    quadx1, quady1 = quad_bbox[1]
                    quadx2, quady2 = quad_bbox[2]
                    quadx3, quady3 = quad_bbox[3]
                ...
                quadx0, quady0 = float(bnd_box.find('xmin').text), float(bnd_box.find('ymin').text)
                quadx1, quady1 = float(bnd_box.find('xmax').text), float(bnd_box.find('ymin').text)
                quadx2, quady2 = float(bnd_box.find('xmax').text), float(bnd_box.find('ymax').text)
                quadx3, quady3 = float(bnd_box.find('xmin').text), float(bnd_box.find('ymax').text)
